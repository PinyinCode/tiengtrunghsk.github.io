...
@keyframes dsNewPulse{
    0%,100%{transform:scale(1);box-shadow:0 2px 8px rgba(220,38,38,.5),0 0 0 2px var(--surface);}
    50%{transform:scale(1.12);box-shadow:0 4px 14px rgba(220,38,38,.8),0 0 0 2px var(--surface);}
}
@keyframes dsNewGlow{
...
@media(min-width:600px){
    .ds-sub-grid{grid-template-columns:repeat(3,1fr);gap:.5rem;}
}
@media(min-width:900px){
    .ds-sub-grid{grid-template-columns:repeat(4,1fr);gap:.5rem;}
}
...
.ds-sub-btn.locked:hover .ds-sub-lock{
    animation:lockShake .5s ease-in-out;
}
@keyframes lockShake{
...
def build_ui_js():
    return r"""
/* ============================================================ */
/* ✅ AUTO VERSION CHECK — Tránh user bị kẹt cache cũ            */
/* ↑↑↑ TĂNG SỐ NÀY MỖI LẦN DEPLOY CODE MỚI ↑↑↑ */
/* ============================================================ */
...
(function checkAppVersion() {
    try {
        ...
        if (storedVersion && storedVersion !== APP_VERSION) {
            ...
            overlay.innerHTML =
                '<div style="width:64px;height:64px;border-radius:16px;background:rgba(255,255,255,.15);display:flex;align-items:center;justify-content:center;font-size:1.8rem;animation:spin 1s linear infinite;backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.2);">' +
                    '<i class="fas fa-sync-alt"></i>' +
                '</div>' +
                '<div style="font-size:1.15rem;font-weight:800;letter-spacing:.3px;">Đang cập nhật phiên bản mới...</div>' +
                '<div style="font-size:.8rem;opacity:.75;font-family:monospace;">' +
